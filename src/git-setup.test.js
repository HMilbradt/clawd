...
describe('git-setup', () => {
  ...
  describe('isGitRepo', () => {
    test('returns true when directory is a git repo', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['rev-parse', '--git-dir'], { cwd: process.cwd() });
    });

    test('returns false when directory is not a git repo', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'not a git repository', 128));
    ...
    test('accepts custom working directory', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['rev-parse', '--git-dir'], { cwd: '/custom/path' });
    });
  });

  describe('initGitRepo', () => {
    test('initializes git repository successfully', async () => {
      spawnMock.mockReturnValue(createMockProcess('Initialized empty Git repository', '', 0));

      await initGitRepo();

      expect(mockLogger.info).toHaveBeenCalledWith('Initializing git repository...');
      expect(spawnMock).toHaveBeenCalledWith('git', ['init'], { cwd: process.cwd() });
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Git repository initialized');
    });

    test('throws error when git init fails', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'Permission denied', 1));

      await expect(initGitRepo()).rejects.toThrow('Failed to initialize git repository: Permission denied');
    });
  });
  ...
  describe('hasCommits', () => {
    test('returns true when repository has commits', async () => {
      ...
      expect(result).toBe(true);
      expect(spawnMock).toHaveBeenCalledWith('git', ['rev-list', '--all', '--count'], { cwd: process.cwd() });
    });
    ...
    test('returns false when command fails', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'fatal: your current branch does not have any commits', 128));
  ...
  describe('createInitialCommit', () => {
    test('skips if repository already has commits', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Repository already has commits');
    });
    ...
    test('creates initial commit when repository is empty', async () => {
      ...
      spawnMock
        ...
        .mockReturnValueOnce(createMockProcess('[main (root-commit)] initial commit', '', 0));

      mockAccess.mockResolvedValue();

      await createInitialCommit();

      expect(mockLogger.info).toHaveBeenCalledWith('Creating initial commit...');
  ...
  describe('getCurrentBranch', () => {
    test('returns current branch name', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['branch', '--show-current'], { cwd: process.cwd() });
    });
  ...
  describe('createBranch', () => {
    test('creates a new branch successfully', async () => {
      spawnMock.mockReturnValue(createMockProcess('Switched to a new branch', '', 0));

      await createBranch('feature-branch');

      expect(mockLogger.info).toHaveBeenCalledWith('Creating branch: feature-branch');
      expect(spawnMock).toHaveBeenCalledWith('git', ['checkout', '-b', 'feature-branch'], { cwd: process.cwd() });
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Branch feature-branch created');
    });

    test('throws error when branch creation fails', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'branch already exists', 128));

      await expect(createBranch('existing-branch')).rejects.toThrow('Failed to create branch existing-branch');
    });
  });

  describe('checkoutBranch', () => {
    test('checks out existing branch', async () => {
      spawnMock.mockReturnValue(createMockProcess('Switched to branch main', '', 0));

      await checkoutBranch('main');

      expect(mockLogger.info).toHaveBeenCalledWith('Switching to branch: main');
      expect(spawnMock).toHaveBeenCalledWith('git', ['checkout', 'main'], { cwd: process.cwd() });
    ...
    test('creates branch if it does not exist', async () => {
      spawnMock
        ...
        .mockReturnValueOnce(createMockProcess('Switched to a new branch', '', 0));

      await checkoutBranch('new-branch');

      expect(spawnMock).toHaveBeenNthCalledWith(1, 'git', ['checkout', 'new-branch'], { cwd: process.cwd() });
      expect(spawnMock).toHaveBeenNthCalledWith(2, 'git', ['checkout', '-b', 'new-branch'], { cwd: process.cwd() });
    });
  });
  ...
  describe('stageAll', () => {
    test('stages all changes successfully', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['add', '-A'], { cwd: process.cwd() });
    });
  ...
  describe('commit', () => {
    test('creates commit successfully', async () => {
      spawnMock.mockReturnValue(createMockProcess('[main abc123] test commit', '', 0));

      await commit('test commit');

      expect(mockLogger.info).toHaveBeenCalledWith('Creating commit: test commit');
      expect(spawnMock).toHaveBeenCalledWith('git', ['commit', '-m', 'test commit'], { cwd: process.cwd() });
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Commit created');
    });

    test('handles nothing to commit gracefully', async () => {
      spawnMock.mockReturnValue(createMockProcess('nothing to commit, working tree clean', '', 1));
    ...
    test('throws error for other commit failures', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'error: pathspec did not match', 1));
  ...
  describe('merge', () => {
    test('merges branch successfully', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['merge', 'feature-branch', '--no-ff'], { cwd: process.cwd() });
    ...
    test('throws error when merge fails', async () => {
      ...
      await expect(merge('feature-branch')).rejects.toThrow('Failed to merge feature-branch');
    });
  });
  ...
  describe('getDiff', () => {
    test('returns diff between branches', async () => {
      ...
      expect(spawnMock).toHaveBeenCalledWith('git', ['diff', 'main...feature'], { cwd: process.cwd() });
    });

    test('throws error when diff fails', async () => {
      spawnMock.mockReturnValue(createMockProcess('', 'error', 1));

      await expect(getDiff('main', 'feature')).rejects.toThrow('Failed to get diff');
    });
  });
  ...
  describe('setupProjectGit', () => {
    test('initializes repo when it does not exist', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Setting up project git repository...');
    ...
    test('skips initialization when repo exists with commits', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Git repository already exists');
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Git setup complete');
    });
  });
});
