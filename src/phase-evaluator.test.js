...
describe('phase-evaluator', () => {
  ...
  describe('evaluatePhase', () => {
    ...
    test('returns complete when all steps are successfully implemented', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Phase evaluation: COMPLETE');
    });
...
Feedback: Tests were not run. Please add and execute tests.`;
    ...
    test('handles no commits in phase branch', async () => {
      ...
      expect(result.incompleteSteps[0].feedback).toContain('No changes were made');
      expect(mockLogger.warn).toHaveBeenCalledWith('No commits found in phase branch');
    });
...
- Dependencies not installed`
    ...
    test('calls git functions with correct parameters', async () => {
      ...
      expect(mockGetCommitsSince).toHaveBeenCalledWith('main-branch', '/custom/path');
      expect(mockGetDiff).toHaveBeenCalledWith('main-branch', 'phase-branch', '/custom/path');
      expect(mockGetCommitMessage).toHaveBeenCalledWith('abc123', '/custom/path');
    ...
    test('passes correct parameters to loadPrompt', async () => {
      ...
      expect(mockLoadPrompt).toHaveBeenCalledWith('phase-evaluation', {
        phaseName: 'Setup',
        stepDescriptions: '1. Initialize project\n2. Create files\n3. Run tests',
    ...
    test('handles multiple commits with proper indexing', async () => {
      ...
      expect(promptCall.commitSummary).toBe('Commit 1: First\nCommit 2: Second\nCommit 3: Third');
    });
...
Feedback: Project initialization is incomplete.`;
    ...
    test('logs phase evaluation progress', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Evaluating phase completion: Setup');
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Phase evaluation: COMPLETE');
    });
...
Feedback: Not done`;
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Phase evaluation: INCOMPLETE');
