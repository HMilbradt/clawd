...
+}`;
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Evaluating step completion: Implement hello function');
      expect(mockLogger.info).toHaveBeenCalledWith('✓ Step evaluation: COMPLETE');
    });
...
Feedback: The function is not implemented, only a TODO comment was added. Please implement the actual hello function.`;
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('Step evaluation: INCOMPLETE');
    });
    ...
    test('returns incomplete when no changes were made', async () => {
      ...
      expect(result.complete).toBe(false);
      expect(result.feedback).toBe('No changes were made. Please implement the required functionality for this step.');
      expect(mockLogger.warn).toHaveBeenCalledWith('No changes detected in step branch');
    });
    ...
    test('uses default feedback when Claude response does not include Feedback section', async () => {
      ...
      expect(result.complete).toBe(false);
      expect(result.feedback).toBe('The step implementation is incomplete. Please review the requirements and ensure all functionality is implemented.');
    });
    ...
    test('passes correct parameters to getDiff', async () => {
      ...
      expect(mockGetDiff).toHaveBeenCalledWith('main-branch', 'feature-branch', '/custom/path');
    });
...
Line 3 of feedback`;
      ...
      const result = await evaluateStep('step', 'main', 'feature', 'brief', 'goal');

      expect(result.feedback).toBe('Line 1 of feedback\nLine 2 of feedback\nLine 3 of feedback');
    });
  });
