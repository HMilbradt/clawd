	...
	while (true) {
		...
		if (interactive && promptQueue && promptQueue.hasPrompts()) {
			...
			if (tui) {
				...
				console.log(
					chalk.green("✓ Prompts processed. Continuing with execution...\n"),
				);
			}
		}
		...
		if (interactive && promptQueue) {
			...
			if (promptQueue.count() > 0) {
				if (tui) {
					...
					console.log(
						chalk.yellow(
							`  (${promptQueue.count()} prompt(s) queued for next iteration)`,
						),
					);
				}
			} else if (tui) {
		...
		const incompleteStepsContext = new Map(); // Map<stepIndex, feedback>
		...
		while (!phaseComplete && phaseAttemptCount < maxPhaseAttempts) {
			...
			for (let stepIndex = 0; stepIndex < phase.steps.length; stepIndex++) {
				...
				if (tui && tui.isCancelRequested()) {
					...
					if (choice === "quit") {
						...
						for (const prompt of prompts) {
							await processInteractivePrompt(
								prompt,
								projectBrief,
								goal,
								phases,
			...
			if (phaseEval.complete) {
				...
				if (tui) {
					tui.log(
						`Phase evaluation: INCOMPLETE (${phaseEval.incompleteSteps.length} steps need work)`,
						"warn",
					);
				} else {
					console.log(
				...
				for (const incomplete of phaseEval.incompleteSteps) {
					...
					if (tui) {
						tui.log(
							`  Step ${incomplete.stepIndex + 1}: ${incomplete.feedback}`,
							"warn",
						);
					} else {
						console.log(
		...
		if (isComplete) {
			...
			if (perpetual) {
				const perpetualMsg =
					"🔄 Perpetual Mode: Researching and adding new features...";
				if (tui) {
...
async function processInteractivePrompt(
	...
	const currentStep = current
		? `Working on phase: ${current.phase.name}`
		: "None";
