...
import { getDiff, getCommitsSince, getCommitMessage, getCommitDiff } from './git-setup.js';
...
export async function evaluatePhase(
  ...
  if (commits.length === 0) {
    ...
    return {
      ...
      incompleteSteps: phase.steps.map((step, index) => ({
        ...
        feedback: 'No changes were made for this step. Please implement the required functionality.'
      }))
    };
  }
  ...
  for (const line of lines) {
    ...
    if (currentStepIndex !== null && line.trim() && !line.startsWith('PHASE_')) {
      currentFeedback += '\n' + line;
    }
  }
