...
export function startInteractiveListener(promptQueue, tui = null) {
		...
		if (key.name === "p") {
			...
			if (tui) {
				...
				if (confirmed) {
					tui.requestCancel();
					tui.log(
						"🛑 Task cancellation requested. Will pause after current task.",
						"warn",
					);
				}
			}
	...
	if (tui) {
		...
		if (screen) {
			...
			screen.key(["space"], () =>
				handleKeypress({ ch: " " }, { name: "space" }),
			);
			screen.key(["escape"], () =>
				handleKeypress({ ch: "\x1b" }, { name: "escape" }),
			);
	...
	try {
		if (tui) {
			...
			const promptType = await select({
				...
				choices: [
					{
						name: "Ask a question about the current status",
						value: "question",
					},
					{ name: "Request a change to the plan", value: "plan_change" },
					{ name: "Add new scope or tasks", value: "add_scope" },
